...
# get number of rows in each process's chunk -
# NOTE: bootcamp ex. use arrays for local and reduced, not sure if I have to
local_nrows = np.array([my_chunk.shape[0]])

# Initialize n_rows object in each
n_rows = np.array([0.])

comm.Reduce(local_nrows, n_rows, op=MPI.SUM)

# Get total number of rows out of array
tot_rows = int(n_rows)

all_chunks = np.zeros((n_cols, tot_rows))

comm.Gatherv(my_chunk.values, [all_chunks, cts, disps, MPI.DOUBLE])
